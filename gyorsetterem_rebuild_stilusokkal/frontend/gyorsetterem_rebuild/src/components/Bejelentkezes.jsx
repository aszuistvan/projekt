    ...
    return (
        <>
            <div className="registration-container h-screen flex items-center bg-white justify-between mr-0">
                <div className="registration-form w-full lg:w-6/12 pt-9 bg-white text-black rounded-lg lg:block hidden">
                    <img src={bejelentkezes_kep} alt="xd" className="justify-start" />
                </div>
                <div className="registration-form w-full lg:w-6/12 bg-white text-black text-center">
                    ...
                    <form onSubmit={handleBejelentkezes} className="flex justify-center">
                        <div className="flex-col-1 -mx-2 justify-center">
                            <div className="relative px-2 py-1">
                                <input type="text" id="email_cim" onChange={e => setEmail(e.target.value)} className="block px-2.5 pb-2.5 pt-4 w-full text-sm text-logozold bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-logozold dark:border-gray-600 dark:focus:border-logozold focus:outline-none focus:ring-0 focus:border-logozold peer border-2 border-gray-300" placeholder=" " />
                                <label htmlFor="floating_outlined" className="absolute text-sm text-logozold dark:text-gray-400 duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] bg-white dark:bg-white px-2 peer-focus:px-2 peer-focus:text-logozold peer-focus:dark:text-logozold peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto start-4">Email-cím</label>
                            </div>
                            <br />
                            <div className="relative px-2 py-1">
                                <input type="password" id="jelszo" onChange={e => setJelszo(e.target.value)} className="block px-2.5 pb-2.5 pt-4 w-full text-sm text-logozold bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-logozold dark:border-gray-600 dark:focus:border-logozold focus:outline-none focus:ring-0 focus:border-logozold peer border-2 border-gray-300" placeholder=" " />
